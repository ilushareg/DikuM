...
void clone_char(struct char_data *ch)
{
	...
	GET_NAME(clone) = strdup_l(GET_NAME(ch));

	clone->player.short_descr =	strdup_l(ch->player.short_descr);

	clone->player.long_descr = strdup_l(ch->player.long_descr);

	clone->player.description = 0;
	/* REMEMBER EXTRA DESCRIPTIONS */

	GET_TITLE(clone) = strdup_l(GET_TITLE(ch));
...
void clone_obj(struct obj_data *obj)
{
	...
	*clone = *obj;
	...
	clone->name               = strdup_l(obj->name);
	clone->description        = strdup_l(obj->description);
	clone->short_description  = strdup_l(obj->short_description);
	clone->action_description = strdup_l(obj->action_description);
