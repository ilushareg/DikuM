...
void load_messages(void)
{
	...
	while(*chk == 'M')
	{
		...
		if(i>=MAX_MESSAGES){
			log_l("Too many combat messages.");
			exit(0);
		}
...
void stop_fighting(struct char_data *ch)
{
	...
	else
	{
		...
		if (!tmp) {
			log_l("Char fighting not found Error (fight.c, stop_fighting)");
			abort();
		}
...
void make_corpse(struct char_data *ch)
{
	...
	corpse->name = strdup_l("corpse");

	sprintf(buf, "Corpse of %s is lying here.", 
	  (IS_NPC(ch) ? ch->player.short_descr : GET_NAME(ch)));
	corpse->description = strdup_l(buf);

	sprintf(buf, "Corpse of %s",
	  (IS_NPC(ch) ? ch->player.short_descr : GET_NAME(ch)));
	corpse->short_description = strdup_l(buf);
...
void hit(struct char_data *ch, struct char_data *victim, int type)
{
	...
	if (ch->in_room != victim->in_room) {
		log_l("NOT SAME ROOM WHEN FIGHTING!");
		return;
	}
