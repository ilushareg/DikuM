...
void night_watchman(void)
{
	...
	if ((t_info->tm_hour == 8) && (t_info->tm_wday > 0) &&
		...
		if (t_info->tm_min > 50)
		{
			log_l("Leaving the scene for the serious folks.");
...
void check_reboot(void)
{
	...
	extern int shutdown_false, reboot_c;
		...
		if (boot = fopen("./reboot", "r"))
		{
			if (t_info->tm_min > 50)
			{
				...
				if (!feof(boot))   /* the file is nonepty */
				{
					...
					if (system("./reboot"))
					{
						log_l("Reboot script terminated abnormally");
...
char *nogames(void)
{
	...
	{
		log_l("/usr/games/nogames exists");
		fgets(text, 200, fl);
		return(text);
		fclose(fl);
	}
...
void gr(int s)
{
	...
	extern int slow_death, shutdown_false;
	...
	if (((ld = load()) >= 6) || (txt = nogames()) || slow_death)
	{
		...
		shutdown_false = 1;				/* this shouldn't happen */
	else if (wnr)
	{
