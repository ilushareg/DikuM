...
void boot_db(void)
{
	...
	log_l("Resetting the game time:");
	reset_time();

	log_l("Reading newsfile, credits, help-page, info and motd.");
	...
	log_l("Opening mobile, object and help files.");
	...
	if (!(help_fl = fopen(HELP_KWRD_FILE, "r")))
      log_l("   Could not open help file.");
	else 
		help_index = build_help_index(help_fl, &top_of_helpt);


	log_l("Loading zone table.");
	boot_zones();

	log_l("Loading rooms.");
	boot_world();
	log_l("Renumbering rooms.");
	renum_world();

	log_l("Generating index tables for mobile and object files.");
	mob_index = generate_indices(mob_f, &top_of_mobt);
	obj_index = generate_indices(obj_f, &top_of_objt);
			
	log_l("Renumbering zone table.");
	renum_zone_table();

	log_l("Generating player index.");
	build_player_index();

	log_l("Loading fight messages.");
	load_messages();

	log_l("Loading social messages.");
	boot_social_messages();

  log_l("Loading pose messages.");
	boot_pose_messages();

	log_l("Assigning function pointers:");
	if (!no_specials)
	{
		log_l("   Mobiles.");
		assign_mobiles();
	...
	log_l("Updating characters with saved items:");
	update_obj_file();
...
void boot_world(void)
{
	...
	{
		perror("fopen");
		log_l("boot_world: could not open world file.");
		exit(0);
	}
...
struct char_data *read_mobile(int nr, int type)
{
	...
	if (letter == 'S') {
		...
		fscanf(mob_f, " %d%*c%d%*c%d ", &tmp, &tmp2, &tmp3);
		//printf("num %ld %ld %ld\n", tmp, tmp2, tmp3);
		mob->points.max_hit = dice(tmp, tmp2)+tmp3;
		mob->points.hit = mob->points.max_hit;

		fscanf(mob_f, " %d%*c%d%*c%d \n", &tmp, &tmp2, &tmp3);
...
void char_to_store(struct char_data *ch, struct char_file_u *st)
{
	...
	if ((i >= MAX_AFFECT) && af && af->next)
		log_l("WARNING: OUT OF STORE ROOM FOR AFFECTED TYPES!!!");
...
char *fread_string(FILE *fl)
{
	...
	do
	{
		...
		{
			log_l("fread_string: string too large (db.c)");
			exit(0);
		}
		else
			strcat(buf, tmp);
	...
	*buf = '\0';
	...
	do
	{
		...
		if (!feof(fl))
		{
			...
			{
				log_l("fl->strng: string too big (db.c, file_to_string)");
				*buf = '\0';
				return(-1);
			}
