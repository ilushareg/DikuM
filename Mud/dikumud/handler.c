...
void affect_modify(struct char_data *ch, byte loc, byte mod, long bitv, bool add)
{
	...
	switch(loc)
	{
		...
		default:
			log_l("Unknown apply adjust attempt (handler.c, affect_modify).");
			break;
...
void affect_remove( struct char_data *ch, struct affected_type *af )
{
	...
	if (ch->affected == af) {
		...
		if (hjp->next != af) {
			log_l("FATAL : Could not locate affected_type in ch->affected. (handler.c, affect_remove)");
			exit(1);
		}
...
void char_from_room(struct char_data *ch)
{
	...
	if (ch->in_room == NOWHERE) {
		log_l("NOWHERE extracting char from room (handler.c, char_from_room)");
		exit(1);
	}
...
void equip_char(struct char_data *ch, struct obj_data *obj, int pos)
{
	...
	if (obj->carried_by) {
		log_l("EQUIP: Obj is carried_by when equip.");
		return;
	}

	if (obj->in_room!=NOWHERE) {
		log_l("EQUIP: Obj is in_room when equip.");
		return;
	}
	...
	if ((IS_OBJ_STAT(obj, ITEM_ANTI_EVIL) && IS_EVIL(ch)) ||
		...
		if (ch->in_room != NOWHERE) {
			...
			log_l("ch->in_room = NOWHERE when equipping char.");
		}
	}
   ...
   if(ch->desc) {
	...
	else
	{
		...
		if(k)
			...
			log_l("Trying to remove ?? from character_list. (handler.c, extract_char)");
			abort();
		}
	}
...
struct obj_data *create_money( int amount )
{
	...
	if(amount<=0)
	{
		log_l("ERROR: Try to create negative money.");
		exit(1);
	}
	...
	if(amount==1)
	{
		obj->name = strdup_l("coin gold");
		obj->short_description = strdup_l("a gold coin");
		obj->description = strdup_l("One miserable gold coin.");

		new_descr->keyword = strdup_l("coin gold");
		new_descr->description = strdup_l("One miserable gold coin.");
	}
	else
	{
		obj->name = strdup_l("coins gold");
		obj->short_description = strdup_l("gold coins");
		obj->description = strdup_l("A pile of gold coins.");

		new_descr->keyword = strdup_l("coins gold");
		if(amount<10) {
			sprintf(buf,"There is %d coins.",amount);
			new_descr->description = strdup_l(buf);
		} 
		else if (amount<100) {
			...
			new_descr->description = strdup_l("There is A LOT of coins");			
	}
