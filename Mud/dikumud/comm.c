...
int shutdown_false = 0;    /* clean shutdown */
int reboot_c = 0;      /* reboot the game after a shutdown */
	...
	while ((pos < argc) && (*(argv[pos]) == '-'))
	{
		switch (*(argv[pos] + 1))
		{
			...
			case 'd':
				...
				{
					log_l("Directory arg expected after option -d.");
					exit(0);
				}
			break;
			case 's':
				no_specials = 1;
				log_l("Suppressing assignment of special routines.");
			break;
			default:
...
int run_the_game(int port)
{
	...
	log_l("Signal trapping.");
	signal_setup();

	log_l("Opening mother connection.");
	s = init_socket(port);

	if (lawful && load() >= 6)
	{
		log_l("System load too high at startup.");
		comas(20);
	}

	boot_db();

	log_l("Entering game loop.");
	...
	log_l("Normal termination of game.");
    return 0;
}
...
int process_input(struct descriptor_data *t)
{
	...
	do
	{
		if ((thisround = read(t->descriptor, t->buf + begin + sofar, 
			...
			{
				log_l("EOF encountered on socket read.");
				return(-1);
			}
	}
...
void close_socket(struct descriptor_data *d)
{
	...
	if (d->character)
		...
		log_l("Losing descriptor without char.");
...
#define COMA_SIGN "\n\rDikuMUD is currently inactive due to excessive load on the host machine.\n\rPlease try again later.\n\r\n\n\r   Sadly,\n\r\n\r    the DikuMUD system operators\n\r\n\r"
...
void comas(int s)
{
	...
	do
	{
		...
		if (FD_ISSET(s, &input_set))
		{
			if (load() < 6)
			{
				log_l("Leaving coma with visitor.");
				sigsetmask(0);
				return;
			}
		...
		if (workhours())
		{
			log_l("Working hours collision during coma. Exit.");
			exit(0);
		}
	}
	while (load() >= 6);
