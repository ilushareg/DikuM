...
void load_char_objs(struct char_data *ch)
{
	...
	if (found) {
       ...
       (unsigned) (((float) st.total_cost*(float) (time(0) - st.last_update))
		...
		if (fwrite(&st, sizeof(struct obj_file_u), 1, fl) < 1) {
			log_l("Error updating name to be set as unused.");
			exit(1);
		}

	} else {
		log_l("Char has no data in file!");
	}
...
void put_obj_in_store(struct obj_data *obj, struct obj_file_u *st)
{
	...
	if (!found) {
		log_l("No empty space to store object. (put_obj_in_store, reception.c)");
		exit(1);
	}
}
